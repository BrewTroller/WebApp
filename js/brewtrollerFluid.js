...
var hosts,
	...
	tempSelectOptions = '<select id="tempSelect">';
	while ($i < 213) {
		tempSelectOptions += "<option value='" + $i + "'>" + $i + "</option>";
		// $('#tempSelect').addOption($i, $i, false);
		$i++;
	}
	tempSelectOptions += '</select>';
	$('#tempDrum').html(tempSelectOptions);
	//$('#tempSelect').drum();

//Setup Segment Display
var hltSegTemp = Brewtroller.segmentDisplay.setup("hltSegTemp", "temp", 40, 25, 4);
var mashSegTemp = Brewtroller.segmentDisplay.setup("mashSegTemp", "temp", 40, 25, 4);
var boilSegTemp = Brewtroller.segmentDisplay.setup("boilSegTemp", "temp", 40, 25, 4);
var mashSegTimer = Brewtroller.segmentDisplay.setup("mashSegTimer", "timer", 40, 25, 4);
var boilSegTimer = Brewtroller.segmentDisplay.setup("boilSegTimer", "timer", 40, 25, 4);
var mashGaugeTimer = Brewtroller.segmentDisplay.setup("mashGaugeTimer", "timer", 12, 10, 2);
var boilGaugeTimer = Brewtroller.segmentDisplay.setup("boilGaugeTimer", "timer", 12, 10, 2);
$('#hltSegTemp').on("click", function (e) {
	$('#tempSetModalTitle').html("Set HLT Temp");
	$('#tempSetModal').modal('toggle');
});
$('#mashSegTemp').on("click", function (e) {
	$('#tempSetModalTitle').html("Set Mash Temp");
	$('#tempSetModal').modal('toggle');
});
$('#hltGauge').on("click", function (e) {
	$('#tempSetModalTitle').html("Set HLT Temp");
	$('#tempSetModal').modal('toggle');
});
$('#mashGauge').on("click", function (e) {
	$('#tempSetModalTitle').html("Set Mash Temp");
	$('#tempSetModal').modal('toggle');
});
$('#div_hltTemperature').on("click", function (e) {
	$('#tempSetModalTitle').html("Set HLT Temp");
	$('#tempSetModal').modal('toggle');
});
$('#div_mashTemperature').on("click", function (e) {
	$('#tempSetModalTitle').html("Set Mash Temp");
	$('#tempSetModal').modal('toggle');
});
