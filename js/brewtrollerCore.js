...
var hosts,
  ...
  Brewtroller.segmentDisplay = {
    setup : function (id, style, height, width, segWidth) {
      display = new SegmentDisplay(id);
      if (style === "temp") {
        display.pattern         = "####.##";
      } else if (style === "timer") {
        display.pattern         = "##:##:##";
      ...
      display.digitHeight     = height;
      display.digitWidth      = width;
      display.digitDistance   = 2.5;
      display.segmentWidth    = segWidth ;
      display.segmentDistance = 0.3;
      display.segmentCount    = 7;
      display.cornerType      = 3;
      display.colorOn         = "#e95d0f";
      display.colorOff        = "#302b28";
      display.draw();
      return(display);
    }
  }
      ...
      brewTrollerExecCommand(BTCMD_GetProgramList, null, null, host, username, password, function(data){
      	$("#recipeList").html("");
				...
				$.each(data, function(index, value) {
					recipeNumber++;
					if (value != ">" && value != "") {
					recipe = '<a class="list-group-item list-group-item-action justify-content-between" href="#">'
	        recipe += '<span>' + recipeNumber + " " + value + '</span>'
	        recipe += '<button>load</button>'
	        recipe += '</a>'
  ...
  Brewtroller.timer = {
    ...
    printTimer : function (id, value, status) {
      ...
      if (id === "#div_mashTimer" || id === "#div_boilTimer") {
      $(id).html('<span class="timerText">' + millisecondsToTime(value) + "</span> / " + rStatus);
    } else {
  ...
  //			$('#button_reset').hide();
  	...
  	printUI : function (data) {
          ...
          // PID Display View Settings
          Brewtroller.status.printTemperature(hltSegTemp, data.HLT_Temperature);
          Brewtroller.status.printTemperature(mashSegTemp, data.Mash_Temperature);
          Brewtroller.status.printTemperature(boilSegTemp, data.Kettle_Temperature);
          Brewtroller.timer.printTimer(mashSegTimer, data.Mash_TimerValue, data.Mash_TimerStatus);
          Brewtroller.timer.printTimer(boilSegTimer, data.Boil_TimerValue, data.Boil_TimerStatus);
          Brewtroller.status.printVolume("#hltSegVolume", data.HLT_Volume);
      		Brewtroller.status.printTargetVolume("#hltSegTargetVolume", data.HLT_TargetVolume);
          Brewtroller.status.printVolume("#mashSegVolume", data.HLT_Volume);
      		Brewtroller.status.printTargetVolume("#mashSegTargetVolume", data.HLT_TargetVolume);
          Brewtroller.status.printVolume("#kettleSegVolume", data.HLT_Volume);
      		Brewtroller.status.printTargetVolume("#kettleSegTargetVolume", data.HLT_TargetVolume);
          // Gauge Display View Settings
          Brewtroller.timer.printTimer(mashGaugeTimer, data.Mash_TimerValue, data.Mash_TimerStatus);
          Brewtroller.timer.printTimer(boilGaugeTimer, data.Boil_TimerValue, data.Boil_TimerStatus);
          Brewtroller.status.printVolume("#hltGaugeVolume", data.HLT_Volume);
      		Brewtroller.status.printTargetVolume("#hltGaugeTargetVolume", data.HLT_TargetVolume);
          Brewtroller.status.printVolume("#mashGaugeVolume", data.HLT_Volume);
      		Brewtroller.status.printTargetVolume("#mashGaugeTargetVolume", data.HLT_TargetVolume);
          Brewtroller.status.printVolume("#kettleGaugeVolume", data.HLT_Volume);
      		Brewtroller.status.printTargetVolume("#kettleGaugeTargetVolume", data.HLT_TargetVolume);
      ...
      printTemperature : function (id, temperature)
      {
          ...
          } else if (id === hltSegTemp || id === mashSegTemp || id === boilSegTemp) {
            temperature = temperature.toFixed(2);
            temperature = temperature.toString();
            var tempSplit = temperature.split(".");
            if (tempSplit[0].length === 2) {
              temperature = "  " + temperature;
            } else {
              temperature = " " + temperature;
            }
            if (id.displayId === "hltSegTemp") {
              hltSegTemp.setValue(temperature);
            } else if (id.displayId === "mashSegTemp") {
              mashSegTemp.setValue(temperature);
            } else if (id.displayId === "boilSegTemp") {
              boilSegTemp.setValue(temperature);
            }
          } else if (id === "#div_hltTemperature" || id === "#div_mashTemperature" || id === "#div_kettleTemperature") {
            $(id).html(temperature);
